...
from __set_os_env__ import MXNET_DATA_COCO
...
def save_params(net, best_map, current_map, epoch, save_interval, prefix, path):
    current_map = float(current_map)
    if current_map > best_map[0]:
        best_map[0] = current_map
        net.save_parameters(os.path.join(path, '{:s}_best.params'.format(prefix, epoch, current_map)))
        with open(os.path.join(path, prefix+'_best_map.log'), 'a') as f:
            f.write('{:04d}:\t{:.4f}\n'.format(epoch, current_map))
    if save_interval and epoch % save_interval == 0:
        net.save_parameters(os.path.join(path, '{:s}_{:04d}_{:.4f}.params'.format(prefix, epoch, current_map)))
